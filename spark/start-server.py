...
def summary(caseNumber):
    ...
    response["originalJudgement"] = json.loads(
        pandasDF["originalJudgement"][0])
...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    INVALID_404_PRECEDENT_REQUEST = {
        "code": "P003", "message": "해당 판례를 찾을수 없습니다."}
    ...
    def do_GET(self):
        ...
        if path == "/api/precedent/summary":
            caseNumber = query_params.get('caseNumber', [None])[
                0]  # None을 기본값으로 사용
            if caseNumber == None:
                self.return_400_error()
                return
