...
# 검색어 리스트
keyword_list = ['법률 개정', '사법 판결', '법정 소송', '규제 변경', '범죄 사건', '인권 보호', '법적 사례', '국제 법률']
...
def crawl_news_each_keyword(keyword): 
    ...
    for i in range(math.ceil( TOTAL_ARTICLE_CNT / ARTICLE_CNT_PER_PAGE )): 
        ...
        crawl_news_per_page( keyword, "{}".format(i * ARTICLE_CNT_PER_PAGE)) # 분기 2 : 페이지 별 뉴스 리스트(10개)를 가져도로록 분산 
...
def get_link_list_by_BeautifulSoup (news_content_html_list):
    ...
    for content in news_content_html_list:
        ...
        soup = BeautifulSoup(content, 'html.parser') # HTML 페이지를 파싱하여 BeautifulSoup 객체 생성
        ...
        try:
            ...
            if article_list :
                ...
                for article in article_list:
                    ...
                    a_tag_list = article.find('div', {'class' : 'info_group'}).find_all('a')
...
def scrap_naver_news(keyword, naverNewsLink):
    ...
    if response.status_code == 200:
        ...
        # 뉴스 제목
        title_element = naver_news_soup.find('h2', class_='media_end_head_headline')
        if title_element:
        ...
        # 뉴스 내용 
        article_content_element = naver_news_soup.find('div', class_='newsct_article _article_body')
        if article_content_element:
            article_content = ''.join([child.text.strip() for child in article_content_element.children if child.name]) # 모든 자식 요소의 innerText를 모두 합치기
        else:
        ...
        if article_img_url_element:
            ...
            article_vedio_img_url_element = naver_news_soup.find('div', class_='_VOD_PLAYER_WRAP')
        ...
        # 작성일시
        temp = naver_news_soup.find(class_='media_end_head_info_datestamp_bunch')
        ...
        # 작성자
        article_writer_element = naver_news_soup.find(class_='media_end_head_journalist_name')
