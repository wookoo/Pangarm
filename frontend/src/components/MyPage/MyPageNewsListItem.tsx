...
interface MyPageNewsListItemProps {
  id: string;
  title: string;
  content: string;
  imageUrl: string;
...
export default function MyPageNewsListItem({
  id,
  title,
  content,
  imageUrl,
  createdAt,
}: MyPageNewsListItemProps) {
  return (
    <div className="mx-3 mb-4 h-44 w-[45%] p-2 shadow-md">
      <Link to={`/news/${id}`}>
        <p className="truncate text-xl font-semibold">{title}</p>
        <p className="mb-3 text-xs text-lightgray">
